...
const roundedLineShader = new THREE.ShaderMaterial ({
    ...
    vertexShader : [
      ...
      "void main(){",
        "vDist = dist;", // passed on to the fragment shader
...
export function roundedPolygonLineGeometry(points, closed : true, w = 1, z = 0.05) {
  ...
  if (lg > 1) {
    ...
    const distArray = new Float32Array( pts );
    ...
    for (let i = 0, p = 0, t = 0; p < lg; i += np, p += 1, t += 2) {
      ...
      const s = Math.sign(d);
      if (np === 5) {
        // add distances to distance array (5 points situation)
    ...
    geometry.addAttribute("dist"    , new THREE.BufferAttribute(distArray  , 1));
