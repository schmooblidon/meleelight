...
export function drawArrayPathCompress(can, col, face, tX, tY, path, scaleX, scaleY, rotate, rpX, rpY) {
    ...
    for (let j = 0; j < path.length; j++) {
        ...
        for (let k = 2; k < path[j].length; k += 6) {
...
export function renderPlayer(i) {
    ...
    const model = animations[cS[i]][player[i].actionState][frame - 1];
    ...
    if (!aS[cS[i]][player[i].actionState].dead) {
        ...
        if (player[i].phys.shielding && player[i].phys.powerShielded && player[i].hit.hitlag > 0) {
            ...
            if (player[i].shocked % 2) {
                const newCol = blendColours(colourArray, [14, 0, 131], 0.7);
            }
            else {
                const newCol = blendColours(colourArray, [255, 255, 255], 0.7);
            }
            ...
            if (part) {
                if (part == 1) {
                    const newCol = blendColours(colourArray, [253, 255, 161], 0.7);
                }
                else {
                    const newCol = blendColours(colourArray, [198, 57, 5], 0.7);
                }
                col = "rgb(" + newCol[0] + "," + newCol[1] + "," + newCol[2] + ")";
            }
            else {
    ...
    if (player[i].phys.shielding) {
        if (!(player[i].phys.powerShielded && player[i].hit.hitlag > 0)) {
            const sX = ((player[i].phys.shieldPositionReal.x) * stage.scale) + stage.offset[0];
            const sY = ((player[i].phys.shieldPositionReal.y) * -stage.scale) + stage.offset[1];
        ...
        for (let j = 0; j < 4; j++) {
            ...
            if (player[i].hitboxes.active[j]) {
                ...
                if (player[i].phys.prevFrameHitboxes.active[j]) {
                    let offset = player[i].phys.prevFrameHitboxes.id[j].offset[player[i].phys.prevFrameHitboxes.frame];
                    if (player[i].actionState == "DAMAGEFLYN") {
