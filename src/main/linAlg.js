...
export function norm(vec : Vec2DType) : number {
  return ( Math.sqrt( dotProd(vec,vec) ));
}


export function squaredDist (center1 : Vec2DType, center2 : Vec2DType) : number {
  return ( (center2.x - center1.x)*(center2.x - center1.x) + (center2.y - center1.y)*(center2.y - center1.y) );
};

export function manhattanDist (center1 : Vec2DType, center2 : Vec2DType) : number {
...
export function orthogonalProjection(point : Vec2DType, line : [Vec2DType, Vec2DType]) : Vec2DType {
  const line0 = line[0];
  const [line0x,line0y] = [line0.x, line0.y];
  // turn everything into relative coordinates with respect to the point line[0]
  const pointVec : Vec2DType = new Vec2D ( point.x - line0x, point.y - line0y);
  const lineVec  : Vec2DType = new Vec2D ( line[1].x - line0x, line[1].y - line0y);
  // renormalise line vector
  const lineNorm : number    = norm(lineVec);
  const lineElem : Vec2DType = scalarProd( 1/lineNorm, lineVec);
  // vector projection calculation
  const factor  : number    = dotProd(pointVec, lineElem);
  const projVec : Vec2DType = scalarProd(factor, lineElem);
